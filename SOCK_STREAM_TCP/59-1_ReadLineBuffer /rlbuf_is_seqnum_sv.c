...
int main(int argc, char* argv[]){
    ...
    uint32_t seqNum;
    struct ReadLineBuffer* rlbuf = malloc (sizeof(struct ReadLineBuffer*));
...
#define ADDRSTRLEN (NI_MAXHOST + NI_MAXSERV + 10)
    ...
    for(;;){
        ...
        bytesRead = readLineBuffer(rlbuf, extBuffer, MAX_EXT_BUF);

        if(bytesRead == -1){
            printf("socket %d gibt %d zurückn", rlbuf->fdcopy, bytesRead);
            close(cfd);
            continue;
        }


        reqLen = atoi(extBuffer);
