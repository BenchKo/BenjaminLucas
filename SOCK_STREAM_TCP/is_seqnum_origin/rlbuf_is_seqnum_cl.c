...
int main(int argc, char* argv[]){
    ...
    struct book_keeping* rlbuf = NULL;
    readLineBufInit(cfd, &rlbuf);
    if (rlbuf == NULL || rlbuf->buffer == NULL) {
        printf("Error: rlbuf or buffer not initialized.\n");
        exit(EXIT_FAILURE);
    }

    numRead = readLineBuffer(rlbuf->fdcopy, rlbuf);
