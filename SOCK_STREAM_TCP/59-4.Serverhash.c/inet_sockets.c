...
/* inet connect verbindet die Aufgaben von getaddrinfo(), sowie von socket() und connect() 
wahrscheinlich nützlich für clients.

Es abstrahiert die Aufgaben und macht den Umgang mit sockets einfacher */
int inetConnect(const char* host, const char* service, int type){
    ...
    hints.ai_socktype = type;   // letzter parameter
...
/* bindet, listened und legt socket optionen fest.  Wird von inetListen() als auch von inetBind() 
aufgerufen*/
...
int inetListen(const char* service, int backlog, socklen_t* addrlen ){
    return inetPassiveSocket(service, SOCK_STREAM, addrlen, TRUE, 0);
}
